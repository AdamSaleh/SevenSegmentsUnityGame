	...
	public class SevenSegments : SevenSegmentsApi 
	{
		...
		protected static IEnumerator PostJsonCoroutine(Uri url, string postdata)
		{
			...
			if (((bool)data_result ["success"])==true) {
				Debug.Log("SevenSegments: Posting "+postdata+" to "+ url.ToString()+" resulted in " + req.text);
			} else {
				Debug.LogError("SevenSegments: Posting "+postdata+" to "+ url.ToString()+" resulted in " + req.text);
			}
		...
		private void StartCoroutine(IEnumerator coroutine){
			if (_coroutineObject == null) {
				var go = new GameObject("SevenSegments Coroutines");
		...
		public SevenSegments (String companyToken, String target, object customer) {
		...
		public SevenSegments (String companyToken, String target, string customer) {
			CompanyToken = companyToken;
			Target = new Uri(target);
